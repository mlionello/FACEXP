...
north_labels_predicted = predict(knn_model,features_north);

Nconfusion_matrices = ...
    confusionmat(north_emo,north_labels_predicted);
acc_north = sum(diag(Nconfusion_matrices))/size(features_north,1)

med_labels_predicted = predict(knn_model,features_med);

Mconfusion_matrices = ...
    confusionmat(med_emo,med_labels_predicted);
acc_med = sum(diag(Mconfusion_matrices))/size(features_med,1)
